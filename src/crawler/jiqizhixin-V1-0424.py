...
class JiqizhixinCrawler:
    """机器之心 AI 新闻爬虫"""
    
    def __init__(self):
        self.name = "机器之心"
        self.url = "https://www.jiqizhixin.com/"
        self.headers = {
    ...
    def fetch(self):
        ...
        try:
            ...
            # 查找新闻列表
            articles = []
            news_items = soup.select('.news-item, .article-item, .list-item, .news-list li')
            
            # 如果没有找到特定选择器，尝试通用选择器
            if not news_items:
                news_items = soup.select('a[href*="/article/"], a[href*="/news/"]')
            ...
            for item in news_items[:10]:  # 最多抓取 10 条
                try:
                    ...
                    url = 'https://www.jiqizhixin.com' + href
            ...
            print(f"✅ 成功抓取 {len(articles)} 条机器之心 AI 新闻")
            return articles
...
def main():
    ...
    # 保存为 JSON
    if articles:
        output_file = f"logs/jiqizhixin_{datetime.now().strftime('%Y%m%d')}.json"
